...
@Suite("EInfoCard.ViewModel")
struct EInfoCardViewModelTests {
    @Test("Сохраняет все поля")
    func storesFields() {
        let common = CommonViewModel(backgroundColor: .red)
        let sut = EInfoCard.ViewModel(
            common: common,
            subtitle: "Total Time",
            value: "12h 15m",
            subtitleColor: .blue,
    ...
    func defaultValues() {
        let sut = EInfoCard.ViewModel(subtitle: "Label", value: "42")
...
@Suite("EInfoCard.Background")
struct EInfoCardBackgroundTests {
    @Test("Background.color сохраняет цвет")
    func colorCase() {
        let bg = EInfoCard.Background.color(.red)
    ...
    func gradientCase() {
        ...
        let bg = EInfoCard.Background.gradient(gradient)
...
@Suite("EInfoCard.Preset")
struct EInfoCardPresetTests {
    @Test("default — задаёт дефолтные цвета и backgroundSecondary")
    func defaultPreset() {
        let sut = EInfoCard.Preset.default(subtitle: "Total Time", value: "12h 15m")
    ...
    func gradientPreset() {
        ...
        let sut = EInfoCard.Preset.gradient(subtitle: "Session", value: "1h 47m", gradient: gradient)
