...
@Suite("ESegmentControl — Snapshots")
struct ESegmentControlSnapshotTests {
    @Test("Два сегмента — первый выбран")
    @MainActor
    func twoSegmentsFirstSelected() {
        let view = ESegmentControl()
        view.configure(with: ESegmentControl.Preset.default(segments: ["Day", "Week"]))
    ...
    func threeSegmentsMiddleSelected() {
        let view = ESegmentControl()
        view.configure(with: ESegmentControl.Preset.default(segments: ["Day", "Week", "Month"]))
        view.select(index: 1)
