...
@Suite("EProgressBar.ViewModel")
struct EProgressBarViewModelTests {
    @Test("Дефолтные значения ViewModel")
    func defaultValues() {
        let sut = EProgressBar.ViewModel()

        #expect(sut.common == CommonViewModel())
        #expect(sut.progress == 0)
        #expect(sut.trackColor == NSColor.Semantic.backgroundTertiary)
        #expect(sut.fillColor == NSColor.Semantic.actionPrimary)
    ...
    func progressClampBelow() {
        let sut = EProgressBar.ViewModel(progress: -0.5)
        #expect(sut.progress == 0)
    }

    @Test("Progress clamp — значение выше 1 становится 1")
    func progressClampAbove() {
        let sut = EProgressBar.ViewModel(progress: 1.5)
        #expect(sut.progress == 1)
    }

    @Test("Progress clamp — граничные значения 0 и 1")
    func progressClampBoundaries() {
        let zero = EProgressBar.ViewModel(progress: 0)
        let one = EProgressBar.ViewModel(progress: 1)
